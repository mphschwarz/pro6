...
class HAL():
    ...
    def spi_open(self):
        ...
        self.client = self.spi.get_port(cs=1, freq=1E3, mode=0)  # SPI Host Verbindung
        write_buf = [0]  # Kommando
        write_buf.extend([0 for i in range(100)])
        self.spi_read_write(write_buf)

        self.client = self.spi.get_port(cs=0, freq=1E3, mode=0)  # SPI Host Verbindung
        write_buf = [0]  # Kommando
        write_buf.extend([0 for i in range(100)])
        self.spi_read_write(write_buf)

        self.client = self.spi.get_port(cs=2, freq=1E3, mode=0)  # SPI Host Verbindung
        write_buf = [0]  # Kommando
        write_buf.extend([0 for i in range(100)])
        self.spi_read_write(write_buf)
